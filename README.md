...
- **최근 변경사항**: 
  ...
  - **🔧 소수점 점수 문제 해결** ⭐⭐⭐⭐⭐ (2025-01-19)
    - **문제**: CSTW 4명 작업자의 점수가 소수점으로 표시 (33.33%, 53.33%, 93.33%)
    - **원인**: 업로드 당시 15개 답변만 저장됨 (정상: 20/25개)
    - **조사**: Excel 파일에는 정상적으로 20/25개 문항 존재, DB 퀴즈도 정상
    - **해결**: 4명 작업자(9452, 9473, 9395, 9487) 데이터 재업로드
    - **결과**: 소수점 점수 → 정수 점수로 정상화
    - **영향 작업자**:
      - 9452 (BRACKET WELD): 15→20 답변, 점수 정규화
      - 9473 (BRACKET FU): 15→20 답변, 점수 정규화
      - 9395 (BLASTING): 15→25 답변, 점수 정규화
      - 9487 (BLASTING): 15→25 답변, 점수 정규화
