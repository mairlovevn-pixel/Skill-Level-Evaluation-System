...
- **마지막 업데이트**: 2025-01-19
- **최근 변경사항**: 
  - **🎨 Level 색상 체계 최종 완성 - 파스텔 톤 + 전체 테두리** ⭐⭐⭐⭐⭐ (2025-01-19)
    - **부드러운 파스텔 색상으로 변경**:
      - **Level 1**: 🤍 흰색 (White) - `rgb(255, 255, 255)` + 회색 테두리
      - **Level 2**: 💛 부드러운 노란색 (Soft Yellow) - `rgb(255, 235, 100)` + 진한 노랑 테두리
      - **Level 3**: 💙 부드러운 파란색 (Soft Blue) - `rgb(100, 150, 255)` + 진한 파랑 테두리
      - **Level 4**: ❤️ 부드러운 빨간색 (Soft Red) - `rgb(255, 100, 100)` + 진한 빨강 테두리
      - **Master (Level 5)**: 🖤 검정색 (Black) - `rgb(0, 0, 0)` + 검정 테두리
    - **모든 Level에 테두리 추가** (1px):
      - Level 1: 회색 테두리 `rgba(100, 100, 100, 1)`
      - Level 2: 진한 노랑 테두리 `rgba(200, 180, 0, 1)`
      - Level 3: 진한 파랑 테두리 `rgba(50, 100, 200, 1)`
      - Level 4: 진한 빨강 테두리 `rgba(200, 50, 50, 1)`
      - Master: 검정 테두리 `rgba(0, 0, 0, 1)`
    - **최적화된 텍스트 색상**:
      - Level 1 (흰색) & Level 2 (노란색): **검은색 텍스트** `#000` (가독성 최고)
      - Level 3, 4, Master: 흰색 텍스트 `#fff`
    - **색상 개선 이유**:
      - 순수 색상 → 너무 쨍해서 눈부심
      - 파스텔 톤 → 부드럽고 전문적인 외관
      - 전체 테두리 → Level 간 명확한 구분
  ...
  - **🔧 필터 시스템 분리 및 확장** ⭐⭐⭐⭐⭐ CRITICAL! (2025-01-19)
    - **두 가지 팀-포지션 구조 분리**:
      - **WRITTEN_TEST_TEAM_POSITIONS** (3개 팀, 18개 포지션):
        - Written Test를 치는 포지션만 포함
        - 사용 탭: **Written Test Results**, **Written Test Analysis**
        - BLACK TOWER (13개): CUTTING, BEVELING, BENDING, LS WELDING, FIT UP, CS WELDING, VTMT, BRACKET FU, BRACKET WELD, UT REPAIR, DOOR FRAME FU, DOOR FRAME WELD, FLATNESS
        - WHITE TOWER (3개): BLASTING, METALIZING, PAINTING
        - INTERNAL MOUNTING (2개): ASSEMBLY, IM CABLE
      - **ALL_TEAM_POSITIONS** (8개 팀, 50개 포지션):
        - Level 평가를 받는 모든 포지션 포함
        - 사용 탭: **Position Analysis**, **Entity Comparison**, **Supervisor Assessment**
        ...
        - LEAN (2개): KAIZEN, EHS
    - **코드 구조 개선**: Written Test용과 Level 평가용 필터를 명확히 분리하여 유지보수성 향상
