...
- ✅ **Level별 법인 현황** (Supervisor Assessment 막대 그래프) - **데이터 레이블 표시**
  - 막대 바깥쪽에 인원 수 표시
  - **법인 필터**: 특정 법인 선택 가능
  - **프로세스 필터**: 특정 프로세스 선택 가능 (NEW!)
  - 두 필터 조합 사용 가능 (법인+프로세스)
...
- **프로세스 선택 기능**: 드롭다운에서 특정 프로세스 선택 시 해당 프로세스 데이터만 표시 (NEW!)
- **Level별 법인 현황**: Supervisor Assessment 레벨별 분포 차트 (법인/프로세스 필터 적용)
- 데이터가 자동으로 업데이트됨
...
- **최근 변경사항**: 
  ...
  - **대시보드 프로세스 필터 추가** (Level별 법인 현황 차트에 프로세스 선택 기능 추가)
...
### 대시보드
- `GET /api/dashboard/stats`: 대시보드 통계 (쿼리 파라미터: `entity`, `processId`)
